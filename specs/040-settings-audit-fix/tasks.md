...
- [ ] T001 [P] [US3] Delete `src/components/settings/SessionsTab.tsx`
- [ ] T002 [P] [US3] Delete `src/components/settings/ActivityTab.tsx`
- [ ] T003 [P] [US3] Delete `src/components/settings/CRMSettingsTab.tsx`
- [ ] T004 [P] [US3] Delete `src/components/settings/AgeBucketEditor.tsx` (verify `useGroupingSettingsStore` and `studentGrouping` config stay intact for directory feature)
- [ ] T005 [US3] Remove dead imports and tab entries from `src/pages/SettingsPage.tsx` — delete any `import` for removed components, remove from tab config array, fix `aria-labelledby` on remaining tabpanels
...
- [ ] T006 [US2] Fix stale closure in UsersTab `useEffect` Escape handler: add `showDeleteConfirm` to dependency array at `src/components/settings/UsersTab.tsx`
- [ ] T007 [US2] Fix inconsistent query params in AuditLogTable failed-attempts section: change empty string `''` to `undefined` for cleared date-from filter, matching login and password-change audit sections
- [ ] T008 [US2] Replace inline `.toLocaleString()` with `formatDate` utility in UsersTab user list rendering
- [ ] T009 [US2] Replace inline `.toLocaleString()` with `formatDate` utility in SessionsActivityTab session list
- [ ] T010 [US2] Replace inline `.toLocaleString()` with `formatDate` utility in AuditLogTable
...
- [ ] T011 [US4] Replace `useCallback` faux-debounce with `useDebounce` hook in UsersTab search (`src/components/settings/UsersTab.tsx`). Pattern: `const debouncedSearch = useDebounce(searchInput, 350)` → pass `debouncedSearch` to `useUsers`
- [ ] T012 [US4] Add `onSuccess` cache invalidation to `useChangePassword` mutation in `src/hooks/useAuthQueries.ts`, matching the pattern of other mutations in the same file
...
- [ ] T013 [P] [US5] Fix a11y in `src/components/settings/ProfileTab.tsx`:
...
- [ ] T014 [P] [US5] Fix a11y in `src/components/settings/SessionsActivityTab.tsx`:
...
- [ ] T015 [P] [US5] Fix a11y in `src/components/settings/UsersTab.tsx`:
...
- [ ] T016 [P] [US5] Fix a11y in `src/components/settings/AuditLogTable.tsx`:
  - Add `scope="col"` on table header `<th>` elements
  - Add `role="status"` on empty state message
- [ ] T017 [US5] Fix `aria-labelledby` in `src/pages/SettingsPage.tsx`: ensure any `<div role="tabpanel">` references existent `id` attributes
...
- [ ] T018 Run `npm run lint` and fix all ESLint errors in modified settings files
- [ ] T019 Run `npm run build` (`tsc -b && vite build`) and verify zero errors
