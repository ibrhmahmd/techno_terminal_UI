            ...
            <div key={paymentType.payment_type} className="border border-slate-200 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={`material-symbols-outlined text-sm transition-transform icon-flip-rtl ${isExpanded ? 'rotate-90' : ''}`} aria-hidden="true">
                    chevron_right
                  </span>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50/50">
                        <th scope="col" className="text-start py-2 px-4 text-xs font-medium text-slate-500">Student</th>
                        <th scope="col" className="text-start py-2 px-4 text-xs font-medium text-slate-500">Group</th>
                        <th scope="col" className="text-end py-2 px-4 text-xs font-medium text-slate-500">Amount</th>
                      </tr>
                    </thead>
                    <tbody>
                      {paymentType.items.map((item, idx) => (
                        <tr key={idx} className="border-b border-slate-100 last:border-0">
                          ...
                          <td className="py-2 px-4 text-sm text-end font-medium text-secondary">
