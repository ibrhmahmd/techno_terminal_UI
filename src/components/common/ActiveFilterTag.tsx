...
export function ActiveFilterTag({ label, value, onRemove }: ActiveFilterTagProps) {
  return (
    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-secondary/10 text-secondary text-sm rounded-full border border-secondary/20 animate-in fade-in slide-in-from-left-2">
      ...
      <button
        onClick={onRemove}
        className="ms-1 w-4 h-4 rounded-full hover:bg-secondary/20 flex items-center justify-center transition-colors focus-visible:ring-2 focus-visible:ring-secondary/30 focus-visible:outline-none"
...
export function ActiveFilterTagsList({
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      ...
      <button
        onClick={onClearAll}
        className="text-sm text-slate-500 hover:text-secondary underline ms-2 focus-visible:ring-2 focus-visible:ring-secondary/30 focus-visible:outline-none rounded"
      >
        Clear all
      </button>
    </div>
